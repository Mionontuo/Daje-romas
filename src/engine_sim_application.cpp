...
void EngineSimApplication::run() {
    while (true) {
        ...
        // Publish once per rendered frame too while paused, so the dashboard
        // immediately reflects pause, ignition and control-state changes.
        publishTelemetry();
...
void EngineSimApplication::publishTelemetry() {
    ...
    if (m_iceEngine == nullptr || m_simulator == nullptr) {
    ...
    snapshot.rpm = m_simulator->filteredEngineSpeed();
